...
uses
  ...
  FileCtrl, Process, IniFiles, DateUtils, Math, dl_era5_settings;
...
begin
 mLog.Clear; //clean the log memo
  ...
  yy0:=1959; //********************LIMIT*************************
  yy0_str:=inttostr(yy0);

  cbDataset.Clear;
  With cbDataset.Items do begin
    add('ERA5 monthly averaged data on single levels from '+yy0_str+' to present');
    add('ERA5 monthly averaged data on pressure levels from '+yy0_str+' to present');
    add('ERA5 hourly data on single levels from '+yy0_str+' to present');
    add('ERA5 hourly data on pressure levels from '+yy0_str+' to present');
  end;
